...
import logging
from metrics import get_user_metrics,get_user_metrics_from_list
...
def main(alg): #main function
    ...
    for dataset in all_datasets:
        ...
        del tr_df
        te_dict = read_from_file(folder_path+'gcn_predictions.txt')
        ...
        for u_ind in te_dict.keys():
            true_m_ind = get_items_from_df(u_ind,te_df)   #get already rated relevant movies only
            pred_m_ind = sorted(te_dict[u_ind],key=te_dict[u_ind].get,reverse=True) #get the prediction movie indxs

            for i in rmd_sz:
                logging.info("Getting user metrics for user id: %d and recommendation size: %d" %(u_ind,i))
                get_user_metrics_from_list(user_metric_dict,u_ind,i,true_m_ind,pred_m_ind,len(true_m_ind))
        logging.info("============================================================================")
        for i in rmd_sz:
